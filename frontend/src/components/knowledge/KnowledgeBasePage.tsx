...
const memories: Memory[] = [
  ...
  },
  {
    ...
    title: '美联储议息规则',
    content:
      'FOMC会议前减少仓位至50%以下，会议结果公布后根据方向再操作。历史波动统计：会议当日平均波动2.3%。',
    tags: ['美联储', '宏观', '仓位管理'],
    importance: 'medium',
    created: '2024-06-20',
    lastAccess: '昨天',
  ...
  return (
    <div className="h-full flex flex-col gap-4 overflow-y-auto">
      ...
      <div className="flex gap-1 bg-[var(--color-surface)] rounded-xl p-1 border border-[var(--color-border)] w-fit">
        <TabButton active={activeTab === 'memories'} onClick={() => setActiveTab('memories')}>
          Agent 记忆
        </TabButton>
        ...
        <div className="grid grid-cols-2 gap-3">
          {memories.map((mem) => {
            ...
            return (
              <div
                key={mem.id}
                className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4 hover:border-[var(--color-primary)]/30 transition-all group"
              >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex gap-1">
                    {mem.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-1.5 py-0.5 rounded bg-[var(--color-surface-hover)] text-[9px] text-[var(--color-text-muted)]"
                      >
                        {tag}
                      </span>
                    ))}
      ...
      {activeTab === 'knowledge' && (
        <div className="grid grid-cols-2 gap-3">
          {knowledgeCards.map((card) => (
            <div
              key={card.id}
              className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4 hover:border-[var(--color-primary)]/30 transition-all cursor-pointer group"
            >
              <div className="flex items-start justify-between mb-2">
                <span className="px-1.5 py-0.5 rounded bg-[var(--color-primary)]/10 text-[9px] text-[var(--color-primary)] font-medium">
                  {card.category}
                </span>
