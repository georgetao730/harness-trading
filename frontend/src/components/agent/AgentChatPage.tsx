                  ...
                  {expandedThinking[msg.id] && (
                    <div className="mt-1.5 space-y-1">
                      {msg.thinking!.map((step, i) => (
                        <div key={i} className="flex items-start gap-2 px-2 py-1 rounded bg-[var(--color-surface-hover)]/50">
                          {step.type === "skill" && <BarChart3 className="w-3 h-3 text-blue-400 mt-0.5" />}
                          {step.type === "reasoning" && <Brain className="w-3 h-3 text-purple-400 mt-0.5" />}
                          {step.type === "risk" && <Shield className="w-3 h-3 text-yellow-400 mt-0.5" />}
                          <div>
                            <p className="text-[10px] font-medium text-[var(--color-text-secondary)]">{step.title}</p>
                            <p className="text-[10px] text-[var(--color-text-muted)]">{step.detail}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <div className="rounded-2xl rounded-bl-md bg-[var(--color-surface)] border border-[var(--color-border)] px-4 py-3">
                  <div className="text-sm text-[var(--color-text-primary)] leading-relaxed prose-sm prose-invert max-w-none">
                    {msg.content.split("\n").map((line, i) => {
                      if (line.startsWith("## ")) {
                        return (
                          <h3 key={i} className="text-sm font-bold mt-2 mb-1 text-[var(--color-text-primary)]">
                            {line.replace("## ", "")}
                          </h3>
                        );
                      }
                      if (line.startsWith("**") && line.endsWith("**")) {
                        return (
                          <p key={i} className="text-xs font-semibold mt-2 mb-0.5 text-[var(--color-text-primary)]">
                            {line.replace(/\*\*/g, "")}
                          </p>
                        );
                      }
                      ...
                      if (line.startsWith("⚠️")) {
                        return (
                          <p key={i} className="text-[10px] mt-2 text-[var(--color-warning)] italic">
                            {line}
                          </p>
                        );
                      }
                ...
                {thinkingSteps.map((step, i) => (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className="text-[11px] text-[var(--color-text-secondary)]">{step.title}</span>
        ...
        <div className="flex gap-2">
          ...
          >
            {isThinking ? (
              ...
              <Send className={cn("w-4 h-4", input.trim() ? "text-white" : "text-[var(--color-text-muted)]")} />
            )}
          </button>
        </div>
      </div>
