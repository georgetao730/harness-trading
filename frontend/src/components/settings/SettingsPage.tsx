...
export function SettingsPage() {
  ...
  const [activeSection, setActiveSection] = useState<'providers' | 'routing' | 'general'>(
    'providers',
  );
                ...
                <div className="px-4 pb-4 border-t border-[var(--color-border)] pt-3 space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <select className="w-full bg-[var(--color-background)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-xs outline-none focus:border-[var(--color-primary)]/50">
                        {availableProviders.map((ap) => (
                          <option key={ap.name} selected={ap.name === provider.provider}>
                            {ap.name}
                          </option>
                        ))}
                      </select>
                    ...
                    <div>
                      ...
                      <select className="w-full bg-[var(--color-background)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-xs outline-none focus:border-[var(--color-primary)]/50">
                        {availableProviders
                          .find((ap) => ap.name === provider.provider)
                          ?.models.map((m) => (
                            <option key={m} selected={m === provider.model}>
                              {m}
                            </option>
                          ))}
                      </select>
        ...
        <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] overflow-hidden">
          ...
          <div className="divide-y divide-[var(--color-border)]">
            {routingRules.map((rule, i) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <span className="text-[10px] text-[var(--color-text-muted)] w-4">
                    P{rule.priority}
                  </span>
